...
当前源码版本及正式Windows单机运行版均为 **0.2.14**。正式目录为 `F:\财务系统\金融计划收费系统_Windows运行版_0.2.14_20260901\FinancialFeeSystem`，继续使用独立数据根 `F:\财务系统\财务数据`并仅监听 `127.0.0.1:8000`；上一正式版0.2.13已停止并移入Windows回收站。0.2.14后端最终回归260/260、数据库核心专项49/49、备份专项52/52、前端生产构建、Windows候选验收、正式备份副本迁移预检和正式切换均已通过。完整发布状态和证据见[项目说明书](docs/项目说明书.md)。开始开发前请先阅读项目说明书、[变更记录](docs/CHANGELOG.md)和[协作规则](AGENTS.md)；它们共同保证不同电脑和不同开发代理可以延续同一套业务与安全边界。
