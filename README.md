...
当前源码及Windows单机运行版为 **0.2.5**。开始开发前请先阅读 [项目说明书](docs/项目说明书.md)、[变更记录](docs/CHANGELOG.md) 和 [协作规则](AGENTS.md)；它们共同保证不同电脑和不同开发代理可以延续同一套业务与安全边界。
...
当前0.2.5延续账户级HWM、Snapshot凭证、Luna隔离和导入复核边界，并完成代码与本地交付清理：未确认导入直接删除，不再制造隔离副本；数据库迁移、备份校验和未知程序错误改为明确的单一路径；构建固定覆盖唯一`release`候选目录，正式验收后本机只保留最新运行版。跨Platform客户Invoice、二态Payment/差额退款和精简FC统计仍在后续批次。
