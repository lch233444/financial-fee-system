...
当前源码版本为 **0.2.19**；正式Windows单机运行版也为 **0.2.19**。正式版位于 `F:\财务系统\金融计划收费系统_Windows运行版_0.2.19_20260904\FinancialFeeSystem`，使用独立数据根 `F:\财务系统\财务数据`并仅监听 `127.0.0.1:8000`。本版修正自然季度的期初边界：Q1/Q2/Q3/Q4仍分别从季度首日开始显示，但首次结算可选上一季末快照作为Beginning，季度首日生效的资金会计入本季，避免漏算；非季度首日开始管理的账户仍使用Starting Date当天快照。资金流水页面同时明确要求填写公司确认的实际入账／基金分配日期，而不是供款所属月份截止日。完整状态和证据见[项目说明书](docs/项目说明书.md)。开始开发前请先阅读项目说明书、[变更记录](docs/CHANGELOG.md)和[协作规则](AGENTS.md)。
