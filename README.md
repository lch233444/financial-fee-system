...
当前源码版本为 **0.2.18**；正式Windows单机运行版也为 **0.2.18**。正式版位于 `F:\财务系统\金融计划收费系统_Windows运行版_0.2.18_20260904\FinancialFeeSystem`，使用独立数据根 `F:\财务系统\财务数据`并仅监听 `127.0.0.1:8000`。0.2.18按用户确认的A+B融合方案升级中英文Invoice PDF：保留清晰、易核对的白底结构，采用深蓝与香槟金的克制金融风格，并把客户、Sub Account、结算期间、服务说明、金额、应付总额和付款资料重新分区；客户PDF不再显示Trustee、Platform或Platform Code。完整状态和证据见[项目说明书](docs/项目说明书.md)。开始开发前请先阅读项目说明书、[变更记录](docs/CHANGELOG.md)和[协作规则](AGENTS.md)。
