...
当前源码及Windows单机运行版为 **0.2.6**；具体切换与验收状态见[项目说明书](docs/项目说明书.md)。开始开发前请先阅读项目说明书、[变更记录](docs/CHANGELOG.md)和[协作规则](AGENTS.md)；它们共同保证不同电脑和不同开发代理可以延续同一套业务与安全边界。
...
当前0.2.6延续账户级HWM、Snapshot凭证、Luna隔离和导入复核边界，并在“季度结算”增加公司内部财务Excel批量筛选、选择及模板导出；公司模板已删除示例数据和个人中文备注。跨Platform客户Invoice、二态Payment/差额退款和精简FC统计仍在后续批次。
