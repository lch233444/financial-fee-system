...
当前源码版本为 **0.2.17**；正式Windows单机运行版也为 **0.2.17**。正式版位于 `F:\财务系统\金融计划收费系统_Windows运行版_0.2.17_20260904\FinancialFeeSystem`，使用独立数据根 `F:\财务系统\财务数据`并仅监听 `127.0.0.1:8000`。0.2.17把整库备份界面升级为按年度、季度标记的完整数据包交接：包内仍是导出时的全部资料，不按季度筛选；相同0.2.17版本可在不同盘符或数据目录导入，系统会重定向受控文件路径并完整覆盖接收方现有资料，不做数据合并。完整状态和证据见[项目说明书](docs/项目说明书.md)。开始开发前请先阅读项目说明书、[变更记录](docs/CHANGELOG.md)和[协作规则](AGENTS.md)。
