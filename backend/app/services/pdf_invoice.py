...
def generate_invoice_pdf(
    *,
    ...
    copy = {
        ...
        "contact": (
            "Please contact the company to confirm payment instructions."
            if is_english else "請聯絡公司確認付款方式。"
        ),
        "page": "Page" if is_english else "頁",
    }
    ...
    due_date = (
        invoice.due_date.strftime("%d %b %Y" if is_english else "%d/%m/%Y")
    ...
    for label, text in (
        ...
        (copy["cheque"], company.cheque_information),
    ):
        if text and text.strip():
