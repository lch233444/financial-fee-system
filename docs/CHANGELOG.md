...
## 0.2.20 - 2026-09-04（Windows运行版）

### 未Finalized资金流水受控更正

- “资金与余额”最近资金流水新增“更正”按钮，可修改资金生效日期、Contribution/Withdrawal类型、金额及备注；必须填写去除首尾空格后2至500字符更正原因。
- 更正保留原Transaction ID和全部凭证关联，在同一写事务新增`TRANSACTION_CORRECTED`审计，记录原因以及更正前后日期、类型、金额分和备注；同内容重复提交会被拒绝，不制造重复审计。
- 当前流水已落入Finalized Settlement期间，或更正后的日期会落入Finalized期间时均返回409；现有SQLite冻结Trigger继续作为底层兜底。未Finalized Draft不会被自动改写，财务更正流水后必须重新Calculate再核对。
- 本批不修改数据库结构、资金计算公式、Settlement/Invoice/Payment、附件文件或Excel母版；Alembic head继续为`d4f8a1c73b29`，Trigger继续为57个。
- 后端完整回归362/362、前端生产构建和Windows候选构建通过；8001纯合成候选完成流水更正、原凭证保留、单条审计、重复更正拒绝、Q2计算及Finalize后锁定，并通过界面验收。
- 切换前由0.2.19生成正式数据包，在F盘隔离数据根完成导入、重启和0.2.20接管预检；正式0.2.20接管8000端口后，同版数据包又在第二个F盘隔离数据根完成全量覆盖导入复验。两次均为`integrity_check=ok`、外键违规0、57个Trigger且无恢复残留。
- 正式0.2.20位于`F:\财务系统\金融计划收费系统_Windows运行版_0.2.20_20260904\FinancialFeeSystem`，已接管`127.0.0.1:8000`及原独立数据根；旧0.2.19正式目录已移入Windows回收站。验收未查询或输出客户业务行，未打开客户文件，未调用Sol/Luna，未修改Excel母版。
