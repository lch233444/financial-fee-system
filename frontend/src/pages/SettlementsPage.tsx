...
import { EmptyState, ErrorBanner, Field, Money, PageHeader, Panel, StatusBadge, SectionNav, Loading } from "../components";
...
export default function SettlementsPage({ notify }: { notify: (message: string) => void }) {
  ...
  const groupAccounts = useMemo(() => !clientId || !platformId || !planId ? [] : accounts.data.filter((account) =>
  ...
  return (
    <>
      ...
      <SectionNav items={[{ id: "settlement-create", label: "建立结算" }, { id: "settlement-history", label: "历史结算" }, { id: "settlement-export", label: "内部Excel" }]} />
