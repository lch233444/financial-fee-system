...
export default function SettlementsPage({ notify }: { notify: (message: string) => void }) {
  ...
  return (
    <>
      <PageHeader title="账单计算" subtitle="每个Sub Account独立计算HWM和Service Fee；历史结余入账后仍须财务人工计算并Finalize" />
      ...
      <Panel id="settlement-create" title="建立结算组合" subtitle="先在上方选择年份、季度和客户，再建立对应的平台与收费计划组合。">
        <p className="settlement-instructions">操作说明：核对本组账户及管理日期，逐账户选择历史结余并核对 HWM → 计算并复核费用及凭证 → Finalize 锁定计算 → 前往“账单出具”签发客户缴费单。首次 HWM 自动取本系统首次结算的期初结余金额；修改需填写原因并单独确认。后续只读继承上期 Next HWM。锁定计算不代表已出具账单。</p>
        <div className="settlement-controls">
