...
import { EmptyState, ErrorBanner, Field, PageHeader, Panel, Loading } from "../components";
...
export default function SetupPage({ notify }: { notify: (message: string) => void }) {
  ...
  function submitFC(event: FormEvent<HTMLFormElement>) {
    return formAction.submit(event, {
      save: (data) => postJson("/api/fcs", { name: data.get("name"), code: data.get("code") }),
      refresh: fcs.reload,
      message: "FC已保存",
    });
  }
  ...
  return (
    <>
      ...
      <div className="tabs" role="group" aria-label="基础资料类别">
        <button aria-pressed={tab === "company"} className={tab === "company" ? "active" : ""} onClick={() => { setTab("company"); setSearch(""); }}>Company</button>
        <button aria-pressed={tab === "fc"} className={tab === "fc" ? "active" : ""} onClick={() => { setTab("fc"); setSearch(""); }}>FC</button>
        <button aria-pressed={tab === "platform"} className={tab === "platform" ? "active" : ""} onClick={() => { setTab("platform"); setSearch(""); }}>Platform</button>
        <button aria-pressed={tab === "plan"} className={tab === "plan" ? "active" : ""} onClick={() => { setTab("plan"); setSearch(""); }}>Fee Plan</button>
      </div>

      {tab === "company" && <div className="split-layout"><Panel title="新增Company" subtitle="Company全名用于账单收款资料；Code仅作内部标识"><form className="form-grid" onSubmit={(e) => void submitCompany(e)}><Field label="Company Name（收款公司全名）"><input name="name" required /></Field><Field label="Company Code（仅内部标识）"><input name="code" required maxLength={20} /></Field><Field label="地址 Address"><input name="address" /></Field><Field label="联系方式 Contact"><input name="contact" /></Field><Field label="默认付款天数"><input name="terms" type="number" defaultValue="14" min="0" max="365" /></Field><Field label="银行收款信息"><textarea name="bank" rows={3} /></Field><Field label="支票信息"><textarea name="cheque" rows={3} /></Field><button className="primary" type="submit" disabled={formAction.pending}>{formAction.pending ? "保存中..." : "保存Company"}</button></form></Panel><Panel title="现有Company"><div className="list-search"><Field label="搜索Company"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="名称或编号…" /></Field></div>{companies.loading ? <Loading /> : companies.data.length ? <div tabIndex={0} role="region" aria-label="可滚动数据表格" className="table-wrap"><table><thead><tr><th>Code</th><th>Name</th><th>付款期限</th><th className="master-actions-column">操作</th></tr></thead><tbody>{companies.data.filter((x) => matchesSearch(`${x.name} ${x.code}`, search)).map((x) => <tr key={x.id}><td><strong>{x.code}</strong></td><td>{x.name}</td><td>{x.payment_terms_days}天</td><td className="master-actions-column">{deleteButton("Company", "/api/companies", x.id, `${x.code} · ${x.name}`)}</td></tr>)}</tbody></table>{!companies.data.some((x) => matchesSearch(`${x.name} ${x.code}`, search)) ? <EmptyState title="没有匹配项" detail="请更换关键词或清空搜索。" /> : null}</div> : <EmptyState title="暂无Company" detail="请先建立公司主体。" />}</Panel></div>}
      {tab === "fc" && <div className="split-layout"><Panel title="新增FC"><form className="form-grid" onSubmit={(e) => void submitFC(e)}><Field label="FC Name"><input name="name" required /></Field><Field label="中介人名字缩写" hint="新建缩写须全系统唯一"><input name="code" required /></Field><button className="primary" type="submit" disabled={formAction.pending}>{formAction.pending ? "保存中..." : "保存FC"}</button></form></Panel><Panel title="现有FC"><div className="list-search"><Field label="搜索FC"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="名称或编号…" /></Field></div>{fcs.loading ? <Loading /> : fcs.data.length ? <div tabIndex={0} role="region" aria-label="可滚动数据表格" className="table-wrap"><table><thead><tr><th>FC</th><th>中介人缩写</th><th className="master-actions-column">操作</th></tr></thead><tbody>{fcs.data.filter((x) => matchesSearch(`${x.name} ${x.code}`, search)).map((x) => <tr key={x.id}><td>{x.name}<small className="cell-note">档案 #{x.id}</small></td><td>{x.code}</td><td className="master-actions-column">{deleteButton("FC", "/api/fcs", x.id, `${x.code} · ${x.name}`)}</td></tr>)}</tbody></table>{!fcs.data.some((x) => matchesSearch(`${x.name} ${x.code}`, search)) ? <EmptyState title="没有匹配项" detail="请更换关键词或清空搜索。" /> : null}</div> : <EmptyState title="暂无FC" detail="建立后可关联客户并统计Service Fee。" />}</Panel></div>}
      {tab === "platform" && <div className="split-layout"><Panel title="新增Platform" subtitle="Trustee是MPF计划受托机构，不是FC/中介人，也不参与Invoice编号"><form className="form-grid" onSubmit={(e) => void submitPlatform(e)}><Field label="Platform Name"><input name="name" required /></Field><Field label="Platform Code"><input name="code" required /></Field><Field label="Trustee（MPF计划受托机构）"><input name="trustee" /></Field><button className="primary" type="submit" disabled={formAction.pending}>{formAction.pending ? "保存中..." : "保存Platform"}</button></form></Panel><Panel title="现有Platform"><div className="list-search"><Field label="搜索Platform"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="名称或编号…" /></Field></div>{platforms.loading ? <Loading /> : platforms.data.length ? <div tabIndex={0} role="region" aria-label="可滚动数据表格" className="table-wrap"><table><thead><tr><th>Code</th><th>Platform</th><th>Trustee（MPF受托机构）</th><th className="master-actions-column">操作</th></tr></thead><tbody>{platforms.data.filter((x) => matchesSearch(`${x.name} ${x.code}`, search)).map((x) => <tr key={x.id}><td>{x.code}</td><td>{x.name}</td><td>{x.trustee || "-"}</td><td className="master-actions-column">{deleteButton("Platform", "/api/platforms", x.id, `${x.code} · ${x.name}`)}</td></tr>)}</tbody></table>{!platforms.data.some((x) => matchesSearch(`${x.name} ${x.code}`, search)) ? <EmptyState title="没有匹配项" detail="请更换关键词或清空搜索。" /> : null}</div> : <EmptyState title="暂无Platform" detail="账单导入也可创建待确认Platform。" />}</Panel></div>}
      {tab === "plan" && <div className="split-layout"><Panel title="新增Fee Plan"><form className="form-grid" onSubmit={(e) => void submitPlan(e)}><Field label="Plan Name"><input name="name" required placeholder="Profit Sharing 20%" /></Field><Field label="Plan Code" hint="新建Code须全系统唯一"><input name="code" required placeholder="PS20" /></Field><Field label="Fee Rate (%)"><input name="rate" type="number" step="0.01" min="0" max="100" defaultValue="20" required /></Field><button className="primary" type="submit" disabled={formAction.pending}>{formAction.pending ? "保存中..." : "保存Fee Plan"}</button></form></Panel><Panel title="现有Fee Plan"><div className="list-search"><Field label="搜索Fee Plan"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="名称或编号…" /></Field></div>{plans.loading ? <Loading /> : plans.data.length ? <div tabIndex={0} role="region" aria-label="可滚动数据表格" className="table-wrap"><table><thead><tr><th>Plan / Code</th><th>Rate</th><th className="master-actions-column">操作</th></tr></thead><tbody>{plans.data.filter((x) => matchesSearch(`${x.name} ${x.code}`, search)).map((x) => <tr key={x.id}><td>{x.name}<small className="cell-note">{x.code} · 档案 #{x.id}</small></td><td>{Number(x.fee_rate_percent).toFixed(2)}%</td><td className="master-actions-column">{deleteButton("Fee Plan", "/api/fee-plans", x.id, `${x.code} · ${x.name}`)}</td></tr>)}</tbody></table>{!plans.data.some((x) => matchesSearch(`${x.name} ${x.code}`, search)) ? <EmptyState title="没有匹配项" detail="请更换关键词或清空搜索。" /> : null}</div> : <EmptyState title="暂无Fee Plan" detail="当前计划可建立为20%高水位线收费。" />}</Panel></div>}
    </>
  );
}
