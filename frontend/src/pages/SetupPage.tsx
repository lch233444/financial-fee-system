...
export default function SetupPage({ notify }: { notify: (message: string) => void }) {
  ...
  return (
    <>
      ...
      {tab === "plan" && <div className="split-layout"><Panel title="新增Fee Plan"><form className="form-grid" onSubmit={(e) => void submitPlan(e)}><Field label="Plan Name"><input name="name" required placeholder="Profit Sharing 20%" /></Field><Field label="Fee Rate (%)"><input name="rate" type="number" step="0.01" min="0" max="100" defaultValue="20" required /></Field><button className="primary" type="submit" disabled={formAction.pending}>{formAction.pending ? "保存中..." : "保存Fee Plan"}</button></form></Panel><Panel title="现有Fee Plan"><div className="list-search"><Field label="搜索Fee Plan"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索名称…" /></Field></div>{plans.loading ? <Loading /> : plans.data.length ? <div tabIndex={0} role="region" aria-label="可滚动数据表格" className="table-wrap"><table><thead><tr><th>收费计划</th><th>Rate</th><th className="master-actions-column">操作</th></tr></thead><tbody>{plans.data.filter((x) => matchesSearch(x.name, search)).map((x) => <tr key={x.id}><td>{x.name}<small className="cell-note">档案 #{x.id}</small></td><td>{Number(x.fee_rate_percent).toFixed(2)}%</td><td className="master-actions-column">{deleteButton("Fee Plan", "/api/fee-plans", x.id, x.name)}</td></tr>)}</tbody></table>{!plans.data.some((x) => matchesSearch(x.name, search)) ? <EmptyState title="没有匹配项" detail="请更换关键词或清空搜索。" /> : null}</div> : <EmptyState title="暂无Fee Plan" detail="当前计划可建立为20%高水位线收费。" />}</Panel></div>}
    </>
  );
}
