    ...
    <>
      ...
      <Panel title="账户数据筛选" subtitle="以下筛选同时作用于资金流水、余额快照及凭证关联记录">
        <div className="settlement-controls transaction-filters">
          <Field label="Client"><select value={filterClientId} onChange={(event) => { setFilterClientId(event.target.value); setFilterAccountId(""); setProofTargetId(""); }}><option value="">全部客户</option>{clientOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
      ...
      <Panel title="最近资金流水">{visibleTransactions.length ? <div className="table-wrap"><table><thead><tr><th>Date</th><th>Client / Platform / A/C</th><th>Type</th><th>Amount</th><th>凭证</th><th>Remark</th></tr></thead><tbody>{visibleTransactions.map((x) => { const account = accountById.get(x.account_id); return <tr key={x.id}><td>{x.transaction_date}</td><td><strong>{account?.client_name || "未知Client"}</strong><small className="cell-note">{account ? [account.platform_name || "待确认Platform", account.account_number, account.scheme_name].filter(Boolean).join(" · ") : x.account_number}</small></td><td>{x.transaction_type}</td><td><Money value={x.amount} /></td><td>{x.evidence_complete ? `完整 (${x.evidence_count})` : "待补"}</td><td>{x.remark || "-"}</td></tr>; })}</tbody></table></div> : <EmptyState title="当前筛选没有资金流水" detail="调整筛选，或为具体Sub Account登记Contribution/Withdrawal。" />}</Panel>
