...
export default function TransactionsPage({ notify }: { notify: (message: string) => void }) {
  ...
  const platformOptions = useMemo(() => Array.from(new Map(accounts.data.filter((item) => item.platform_id != null && (!filterClientId || item.client_id === Number(filterClientId))).map((item) => [item.platform_id as number, item.platform_name || "待确认Platform"])).entries())
  ...
  return (
    <>
      ...
      <Panel id="cash-filters" title="账户数据筛选" subtitle="筛选资金流水、余额快照及凭证记录；新增记录时请单独核对账户">
        <div className="settlement-controls transaction-filters">
          <Field group label="Client"><SearchableSelect label="资金与余额客户" value={filterClientId} onChange={(value) => { setFilterClientId(value); setFilterPlatformId(""); setFilterAccountId(""); setProofTargetId(""); }} placeholder="全部客户" options={clientOptions.map((item) => ({ value: String(item.id), label: item.name }))} /></Field>
